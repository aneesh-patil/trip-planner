...
data.forEach((dest) => {
  ...
  let tickets = dest.categories.includes("Theme Park")
    ? 8000
    : dest.categories.includes("Museums") || dest.categories.includes("Art")
      ? 2000
      : dest.categories.includes("Onsen")
        ? 1500
        : 0;
...
console.log(
  "Successfully updated destinations.json with budget breakdowns and comfort metrics.",
);
