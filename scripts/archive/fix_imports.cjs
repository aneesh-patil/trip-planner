...
const replacements = [
  {
    from: /@\/pages\/(Home|Destinations|DestinationDetails|Compare|Favorites|JapanMap)/g,
    to: (match, p1) => {
      const featureMap = {
        Home: "home",
  ...
  {
    from: /@\/components\/destinations/g,
    to: "@/features/destinations/components",
  },
  { from: /@\/hooks/g, to: "@/shared/hooks" },
  { from: /@\/lib\/utils/g, to: "@/shared/utils/utils" },
  { from: /@\/types/g, to: "@/shared/types" },
  {
    from: /@\/data\/destinations\.json/g,
    to: "@/shared/data/destinations-index.json",
  },
];
...
walk(path.join(__dirname, "../src"), (filePath) => {
  ...
  if (filePath.endsWith("App.tsx")) {
    ...
    content = content.replace(
      /import\("\.\/features\/DestinationDetails"\)/g,
      'import("./features/destinations/DestinationDetails")',
    );
    content = content.replace(
      /import\("\.\/features\/Compare"\)/g,
      'import("./features/compare/Compare")',
    );
    content = content.replace(
      /import\("\.\/features\/JapanMap"\)/g,
      'import("./features/map/JapanMap")',
    );
    content = content.replace(
      /import Home from "\.\/features\/Home";/g,
      'import Home from "./features/home/Home";',
    );
    content = content.replace(
      /import Destinations from "\.\/features\/Destinations";/g,
      'import Destinations from "./features/destinations/Destinations";',
    );
    content = content.replace(
      /import Favorites from "\.\/features\/Favorites";/g,
