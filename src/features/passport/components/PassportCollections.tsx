...
export function PassportCollections() {
  const { visited } = useTripStore();

  const collections = collectionsIndex as Collection[];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2.5">
            ...
            Curated Collections Progress
          </h2>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">
            Progress metrics across official UNESCO sites and regional Japanese
            travel themes
          </p>
        </div>
      </div>
          ...
          ({ collection, visitedCount, totalMembers, pct, isCompleted }) => {
            ...
            return (
              <Link
                ...
                className="group block p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-teal-500/50 transition-all shadow-sm"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="p-2 rounded-xl bg-white dark:bg-slate-700 text-teal-600 dark:text-teal-400 shadow-sm group-hover:scale-110 transition-transform shrink-0">
                  ...
                  <div className="w-full h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${
                        isCompleted ? "bg-teal-500 shadow-sm" : "bg-teal-500"
                      }`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
