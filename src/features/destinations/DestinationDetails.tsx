...
import { useWeekendWeather, getWeatherDescription } from "@/shared/hooks/useWeather";
...
export default function DestinationDetails() {
  ...
  useEffect(() => {
    if (id) {
      setDestLoading(true);
      destinationService.getDestination(id).then((dest: Destination | null) => {
        setDestination(dest);
                        ...
                        {destination.transportOptions?.train && (
                          <div className="flex justify-between items-center text-sm border-b border-slate-100 dark:border-slate-800 pb-2">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-slate-400">est. ¥{(budgetService.getTransportCost(destination, 'train') / 1000).toFixed(1)}k</div>
                            </div>
                          </div>
                        )}
                        {destination.transportOptions?.shinkansen && (
                          <div className="flex justify-between items-center text-sm border-b border-slate-100 dark:border-slate-800 pb-2">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-slate-400">est. ¥{(budgetService.getTransportCost(destination, 'shinkansen') / 1000).toFixed(1)}k</div>
                            </div>
                          </div>
                        )}
                        {destination.transportOptions?.bus && (
                          <div className="flex justify-between items-center text-sm border-b border-slate-100 dark:border-slate-800 pb-2">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-slate-400">est. ¥{(budgetService.getTransportCost(destination, 'bus') / 1000).toFixed(1)}k</div>
                            </div>
                          </div>
                        )}
                        {destination.transportOptions?.car && (
                          <div className="flex justify-between items-center text-sm border-b border-slate-100 dark:border-slate-800 pb-2">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-slate-400">est. ¥{(budgetService.getTransportCost(destination, 'car') / 1000).toFixed(1)}k</div>
                            </div>
                          </div>
                        )}
                    ...
                    <CardContent className="p-5 flex flex-col h-full">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          ...
                          <div className="text-emerald-600 font-black text-lg">¥{budgetService.getAdjustedBudget(destination, selectedTransport).toLocaleString()}</div>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2 mt-auto">
                          <div className="flex justify-between text-sm border-b border-slate-100 dark:border-slate-800 pb-1.5 mt-1.5 first:mt-0">
                            ...
                            <span className="font-semibold text-slate-700 dark:text-slate-300">
                              ¥{budgetService.getTransportCost(destination, selectedTransport).toLocaleString()}
                            </span>
                          </div>
