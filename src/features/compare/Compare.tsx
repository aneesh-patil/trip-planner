...
import { Badge } from "@/shared/components/ui/badge";import { getAdjustedBudget } from "@/shared/utils/utils";
  ...
  if (compareDestinations.length === 0) {
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold tracking-tight mb-8">Compare Destinations</h1>
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Nothing to compare</h3>
          <p className="text-slate-500 mb-6 text-center max-w-md">Add destinations to your comparison list from the explore page to see them side-by-side.</p>
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Compare Destinations</h1>
          <p className="text-slate-500 mt-1">Side-by-side analysis to help you decide.</p>
        </div>
        <div className="flex gap-2">
          {compareDestinations.length < 4 && (
            <Link to="/destinations">
              <Button variant="outline" className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-900 dark:text-emerald-400 dark:hover:bg-emerald-900/30">
                <PlusSquare className="w-4 h-4 mr-2" /> Add more
              </Button>
            </Link>
          )}
          <Button variant="outline" className="text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30" onClick={clearCompare}>
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              <TableHead className="w-[200px] align-top py-6">Features</TableHead>
              {compareDestinations.map(dest => (
                <TableHead key={dest.id} className="min-w-[200px] align-top py-6 relative group">
                  <button 
                  ...
                  <div className="relative">
                    <img src={dest.heroImage} alt={dest.name} className="w-full h-32 object-cover rounded-md mb-3" />
                  </div>
          ...
          <TableBody>
            <TableRow>
              <TableCell className="font-semibold text-slate-700 dark:text-slate-300">Overall Score</TableCell>
              {compareDestinations.map(dest => (
                <TableCell key={dest.id} className="text-base">
                  <span className={`font-bold ${dest.ratings.overall === maxOverall ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
                    {dest.ratings.overall}
                  </span>
                  {dest.ratings.overall === maxOverall && <Badge className="ml-2 bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800">Best</Badge>}
                </TableCell>
              ))}
            </TableRow>
            <TableRow>
            ...
            <TableRow>
              <TableCell className="font-semibold text-slate-700 dark:text-slate-300">Walking Required</TableCell>
              {compareDestinations.map(dest => (
                <TableCell key={dest.id}>
                  <span className={dest.walkingMin === minWalking ? "font-bold text-emerald-600 dark:text-emerald-400" : ""}>
                    {(dest.walkingMin / 1000).toFixed(1)}k steps
                  </span>
                  {dest.walkingMin === minWalking && <Badge className="ml-2 bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800">Least</Badge>}
                </TableCell>
              ))}
            </TableRow>
            <TableRow>
              <TableCell className="font-semibold text-slate-700 dark:text-slate-300">Couple Score</TableCell>
              {compareDestinations.map(dest => (
                <TableCell key={dest.id}>
                  <span className={dest.ratings.couple === maxCoupleScore ? "font-bold text-emerald-600 dark:text-emerald-400" : ""}>
                    {dest.ratings.couple}/10
                  </span>
                  {dest.ratings.couple === maxCoupleScore && <Badge className="ml-2 bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800">Highest</Badge>}
                </TableCell>
              ))}
            </TableRow>
            <TableRow>
              <TableCell className="font-semibold text-slate-700 dark:text-slate-300">Summer Comfort</TableCell>
              {compareDestinations.map(dest => (
                <TableCell key={dest.id}>
                  <span className={dest.ratings.summer === maxSummerScore ? "font-bold text-emerald-600 dark:text-emerald-400" : ""}>
                    {dest.ratings.summer}/10
                  </span>
                  {dest.ratings.summer === maxSummerScore && <Badge className="ml-2 bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800">Best</Badge>}
                </TableCell>
              ))}
            </TableRow>
            <TableRow>
              <TableCell className="font-semibold text-slate-700 dark:text-slate-300">Vibe Tags</TableCell>
              {compareDestinations.map(dest => (
                <TableCell key={dest.id}>
                  <div className="flex flex-wrap gap-1">
                    {dest.tags.map(tag => (
                      <span key={tag} className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-md">{tag}</span>
                    ))}
                  </div>
                </TableCell>
              ))}
