...
class WeatherService {
  private cache = new Map<string, { timestamp: number; data: WeatherForecast[] }>();

  public getWeatherDescription(code: number): { text: string; icon: "sun" | "cloud" | "rain" | "snow" | "storm" } {
  ...
  public async getWeekendWeather(lat: number, lng: number): Promise<WeatherForecast[]> {
    const cacheKey = `${lat.toFixed(2)},${lng.toFixed(2)}`;
    const now = Date.now();
    const cached = this.cache.get(cacheKey);
    
    if (cached && (now - cached.timestamp < APP_CONFIG.weatherApi.cacheDurationMs)) {
      return cached.data;
    }

    const res = await fetch(`${APP_CONFIG.weatherApi.baseUrl}/forecast?latitude=${lat}&longitude=${lng}&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=Asia%2FTokyo`);
