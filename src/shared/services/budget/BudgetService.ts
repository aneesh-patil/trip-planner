...
class BudgetService {
  private getRentalBaseFee(tripDurationHours: number): number {
  ...
  public getTransportCost(dest: Destination, mode: string): number {
    const PARTY_SIZE = APP_CONFIG.multipliers.couple;
    ...
    if (mode === "bus" && dest.transportOptions?.bus) {
      ...
      const perPersonRoundTrip = Math.floor(2000 + time * 15);
      return perPersonRoundTrip * PARTY_SIZE;
    }
    ...
    if (mode === "car" && dest.transportOptions?.car) {
      ...
      const rentalFee = this.getRentalBaseFee(tripDurationHours);
  ...
  public getAdjustedBudget(dest: Destination, activeMode: string): number {
    let mode = "train";
    ...
    const transportCost = this.getTransportCost(dest, mode);
    const otherCosts =
